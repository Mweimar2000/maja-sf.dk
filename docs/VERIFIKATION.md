...
| Drift | Kørende version, analyseefterslæb og kladde verificeres i Apps Script | 8.1.4 installeret og afprøvet; faktatjek ramte tidsgrænsen. 8.1.5 afventer afsluttende driftsprøve |
...
Den første pakke på 20 kontroller gav 18 fejl mod Claude-versionen `df0b6394ab853e3864dec132913c07224fd9de75`, og alle 20 bestod efter rettelse. Flere tests er siden tilføjet for indsamling og genforsøg. **Seneste lokale kørsel: 136/136 bestået** (Node 24.19.0). `node --test` viser det aktuelle samlede antal.
...
- Apps Script kører `8.1.4-validation`, identisk med commit `dd5fa78` efter normalisering af linjeskift. Hele regnearket, hovedfilen, to hjælpefiler og manifestet er sikkerhedskopieret. 8.1.5 er under lokal kontrol og ikke installeret endnu.
...
Den samlede reparation er endnu ikke afsluttet. Næste trin er at afprøve det genoptagelige faktatjek i drift, kontrollere rapporten og derefter opdatere og kontrollere de tre robottriggere.
...
[Googles kvoter](https://developers.google.com/apps-script/guides/services/quotas) angiver seks minutter pr. kørsel og 90 minutters samlet triggertid pr. døgn for forbrugerkonti. [Engangstriggerens after-metode](https://developers.google.com/apps-script/reference/script/clock-trigger-builder#after(Integer)) angiver en minimumsforsinkelse; den faktiske start kan ligge senere. Den nye opdeling er under lokal afprøvning og er ikke installeret endnu.


Den separate testgennemgang af 8.1.5 bestod alle 136 tests (51 unit tests og 85 integrationstests). Den uafhængige kodegennemgang afsluttedes uden udestående P1/P2-fund. Testene afbryder simulerede eksekveringer under modelkald og rapportgemning, kontrollerer faste genforsøgsgrænser, gennemfører køen i nye eksekveringer og bevarer tekst samt billeder/formatering efter afbrydelse. En kladde læses eller ændres aldrig af senere workers. Dette er lokalt driftsbevis med simulerede tjenester; Google-prøven udestår fortsat.
