...
| Drift | Kørende version, analyseefterslæb og kladde verificeres i Apps Script | 8.1.9 installeret og fuldt genlæst; fem MOTAS-PDF’er accepteret via countTokens; den ældre 42-bilagskø er afsluttet med 14/14 kontroller. Rigtig 8.1.8-skrivetekst er fortsat udestående efter HTTP 429 på alle tre produktionsmodeller |
...
Den første pakke på 20 kontroller gav 18 fejl mod Claude-versionen `df0b6394ab853e3864dec132913c07224fd9de75`, og alle 20 bestod efter rettelse. Flere tests er siden tilføjet for indsamling og genforsøg. **Seneste lokale kørsel: 240/240 bestået** for 8.1.9-leverancen. De 23 nye PDF-/størrelsesprøver giver 13 fejl mod 8.1.8-baselinen. Paulis afsluttende uafhængige review fandt ingen udestående P1/P2-fejl. `node --test` viser det aktuelle samlede antal.
...
Apps Script kører `8.1.9-validation`, genlæst i sin helhed kl. 12.23. SHA-256: `47f6ca1dd51352d2ce950edad398514863f156ae6d46e9e5fbb36c178a2455d6`. 240/240 lokale tests består. Det uafhængige review af denne PDF-rettelse fandt ingen udestående P1/P2-fejl.

Den seneste arkdiagnose kl. 11.48 viser 1.185 datarækker, 147 bevarede historiske dagsordener og 432 aktive manglende/ugyldige analyser, heraf 108 i det aktuelle nyhedsvindue. I alt 14 analyser er repareret: den faktisk planlagte genforsøgskørsel kl. 11.09–11.11 gennemførte de seneste fem efter et låseafslag kl. 11.01. Det er observeret drift, ikke en simulation. Den aktive testkø fra 8.1.7 var kl. 12.15 nået til 15 af 43 PDF-opgaver; den bevarer sit job og sender ingen mail.
...
8.1.9 tillader 30 MiB pr. PDF og samlet som dekodet input. Alle Gemini-kald kontrollerer den færdige JSON-anmodnings præcise UTF-8-størrelse efter tilføjelse af systeminstruktion; grænsen er 45 MiB. For store requests stoppes før netværk uden model-fallback. MIME- og signaturkrav bevares med særskilte fejl. ZIP- og mailgrænser er uændrede. Tests omfatter den konkrete femfilsstørrelse, alle bilags bevarelse, forkert type/signatur, padding, Unicode, systeminstruktion og nul netværkskald ved afvisning.

Grænserne er holdt under [Apps Scripts 50 MB POST-grænse](https://developers.google.com/apps-script/guides/services/quotas) og den aktuelle [Gemini-dokumentation for inline-input](https://ai.google.dev/gemini-api/docs/generate-content/file-input-methods). [PDF-dokumentationen](https://ai.google.dev/gemini-api/docs/generate-content/document-processing) og den faktiske tokenprøve er kontrolleret særskilt. Større sager afvises fortsat synligt.

En ny rigtig nyhedsbrevstekst med statusrettelserne er fortsat udestående efter kvotefejl på de tre produktionsmodeller og to overbelastede 3.8-prøver. Ingen model eller betalingsindstilling er ændret. Den afsluttede ældre faktatjekkø er ikke bevis for en ny, godkendt skrivning.

## Tidligere driftsprøver 9. september 2026

- Apps Script kører `8.1.8-validation`, installeret 9. september kl. 11.16 og genlæst i sin helhed. Hele hovedfilen matcher den endelige leverance efter normalisering af linjeskift: SHA-256 `00227f9452351649d2337a86a4ed212347e6e5b8413255f10a98eeb5e5edd36f`. 217/217 lokale tests består, og Paulis sidste review af denne leverance har ingen udestående P1/P2-fund. Regneark, hovedfil, to hjælpefiler og manifest er sikkerhedskopieret; den private rapportmappe og projektets skjulte manifestvisning er tidligere kontrolleret.
...
8.1.9-leverancen består af PDF-rettelsen, 23 nye regressionstests og opdateret driftsdokumentation. Hovedfilen i Apps Script er genlæst og matcher SHA-256 ovenfor. [GitHub Actions](https://github.com/Mweimar2000/maja-sf.dk/actions/workflows/robot-tests.yml) viser CI-resultater for den konkrete publicerede commit; et tidligere grønt resultat gælder ikke automatisk en senere commit.
