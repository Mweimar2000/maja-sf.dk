...
| Drift | Kørende version, analyseefterslæb og kladde verificeres i Apps Script | 8.1.8 installeret og fuldt genlæst; den ældre 42-bilagskø er afsluttet med 14/14 kontroller. Rigtig 8.1.8-skrivetekst er fortsat udestående efter HTTP 429 på alle tre produktionsmodeller |
...
**Status: 8.1.8 installeret; publicering verificeres separat.** Ved seneste kontrol gav PR #5 HTTP 404, og repositoryets Settings viste, at Pull requests-funktionen var slået fra. Arbejdsbranchen var bekræftet tilgængelig (commitpræfiks `c9fc`). Den efterfølgende træsammenligning viste identisk indhold i `main` (`80d`) og merge-basen (`85d`): ingen indholdsændringer, som kun findes på `main`, skal bevares ud over featuretræet. Dette understøtter den forberedte merge med både `main` og featurebranchen som forældre, men er ikke bevis for en gennemført publicering. Den endelige publicering skal verificeres særskilt; en tilgængelig PR eller et afsluttet merge påstås ikke her. GitHub-status er adskilt fra den bekræftede Apps Script-installation, og et merge deployer ikke koden til Apps Script.
